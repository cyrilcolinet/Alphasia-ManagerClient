        ...
        while (enabled) {
            try {
                ...
                try {
                    ...
                    AlphaManagerClient.log(Level.ERROR, "Exception during subscription. " + e.getMessage());
                }

                AlphaManagerClient.log(Level.INFO, "Closing subscription. " + (((!enabled) ? "Stopping thread." : "Trying to restart.")));
                jedis.close();
            } catch (Exception e) {
                AlphaManagerClient.log(Level.ERROR, "Exception while getting database: " + e.getMessage() + ". Retrying in 5 seconds.");
